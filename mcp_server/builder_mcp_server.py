...
models_path = os.path.join(root_dir,Config.assets_path,"models")
...
def get_available_assets()->str:
    """
    工具名：获取可用的3D模型资产列表
    描述：该工具扫描本地 assets/models 目录，返回目录下由视觉大模型清洗、SAM3D生成的3D模型资产文件（.glb）以及对应的空间坐标（JSON）
    返回值：包含资产名称和空间位置的JSON字符串
    参数：无
    """
